...
def generate_launch_description():
    ...
    robot_description = {
        "robot_description": load_text_or_xacro(urdf_path)
    }

    robot_description_semantic = {
        "robot_description_semantic": load_text_or_xacro(srdf_path)
    }

    kinematics_yaml = {
        "robot_description_kinematics": load_yaml(
            os.path.join(config_dir, "kinematics.yaml")
        )
    }

    joint_limits_yaml = {
        "robot_description_planning": load_yaml(
            os.path.join(config_dir, "joint_limits.yaml")
        )
    }
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("x", default_value="0.30"),
        DeclareLaunchArgument("y", default_value="0.00"),
        DeclareLaunchArgument("z", default_value="0.60"),

        DeclareLaunchArgument("roll", default_value="0.0"),
        DeclareLaunchArgument("pitch", default_value="1.57"),
        DeclareLaunchArgument("yaw", default_value="0.0"),

        DeclareLaunchArgument("start_x", default_value=""),
        DeclareLaunchArgument("start_y", default_value=""),
        DeclareLaunchArgument("start_z", default_value=""),
        DeclareLaunchArgument("start_roll", default_value="0.0"),
        DeclareLaunchArgument("start_pitch", default_value="0.0"),
        DeclareLaunchArgument("start_yaw", default_value="0.0"),

        DeclareLaunchArgument("step_size", default_value="0.02"),
        DeclareLaunchArgument("num_waypoints", default_value="0"),
        DeclareLaunchArgument("total_time", default_value="0.0"),

        DeclareLaunchArgument("execute", default_value="0"),

        Node(
            package="erobot_motion_client",
            executable="linear_interpolation_client",
            name="linear_interpolation_client",
            output="screen",
            arguments=[
                ...
                "--total-time", LaunchConfiguration("total_time"),
