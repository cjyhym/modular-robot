  ...
  const rclcpp_lifecycle::State &)
{
  ...
  constexpr unsigned int STARTUP_WAIT_CYCLES = 30000;

  for (
    unsigned int cycle = 0;
    cycle < STARTUP_WAIT_CYCLES;
    ++cycle)
  {
    ...
    for (auto & joint : joints_) {
      ...
      joint.status_word = EC_READ_U16(
        domain_data_ + joint.offset_status_word);
      joint.digital_inputs = EC_READ_U32(
        domain_data_ + joint.offset_digital_inputs);
      ...
      EC_WRITE_U32(
        domain_data_ + joint.offset_digital_outputs,
    ...
    if (cycle % 1000U == 0U) {
      ...
      for (auto & diagnostic_joint : joints_) {
        ec_slave_config_state_t slave_state{};

        if (diagnostic_joint.slave_config != nullptr) {
          ecrt_slave_config_state(
            diagnostic_joint.slave_config,
            &slave_state);
        }
        ...
        RCLCPP_INFO(
          ...
          "al_state=0x%02X "
          "status_word=0x%04X",
          diagnostic_joint.name.c_str(),
          diagnostic_joint.slave_position,
          slave_state.online,
          slave_state.operational,
          slave_state.al_state,
          diagnostic_joint.status_word);
      }
    }

    if (domain_state_.wc_state == EC_WC_COMPLETE &&
        all_status_valid)
    {
      ++stable_cycles;
      if (stable_cycles >= 20) {
  ...
  if (!ready) {
    RCLCPP_ERROR(
      logger_,
      "六轴PDO在30秒内未稳定：wc_state=%u working_counter=%u",
  ...
  for (auto & joint : joints_) {
    joint.position = counts_to_radians(joint, joint.actual_count);
    joint.previous_position = joint.position;
    joint.velocity = 0.0;
    joint.command = joint.position;
    joint.applied_command = joint.position;
    joint.target_count = joint.actual_count;
    joint.initialized = true;

    RCLCPP_INFO(
      logger_,
      "%s初始化：slave%u count=%d position=%.9f rad",
      joint.name.c_str(),
      joint.slave_position,
      joint.actual_count,
      joint.position);
  ...
  if (!enable_drives_) {
    RCLCPP_WARN(
      logger_,
      "六轴硬件已激活，但enable_drives=false，驱动不会进入Operation Enabled");
  } else {
    RCLCPP_WARN(
      logger_,
      "enable_drives=true，六个驱动将按CiA402状态机使能并保持当前位置");
  }
  ...
  const rclcpp::Duration & period)
{
  ...
  if (wc_state != last_wc_state_) {
    RCLCPP_INFO(
      logger_,
      "Domain WC状态变化：%u -> %u，working_counter=%u",
      last_wc_state_,
      wc_state,
      domain_state_.working_counter);
  ...
  if (domain_state_.wc_state != EC_WC_COMPLETE) {
    ...
    if (incomplete_wc_cycles_ == 1 ||
        incomplete_wc_cycles_ == 1000)
    {
      RCLCPP_WARN(
        logger_,
        "Domain WC不完整：连续%u周期",
    ...
    if (incomplete_wc_cycles_ > wc_error_limit_) {
      RCLCPP_ERROR(logger_, "Domain WC持续不完整，进入安全停止");
      safety_stop_ = true;
  ...
  for (auto & joint : joints_) {
    ...
    if (joint.status_word != joint.last_status_word) {
      RCLCPP_INFO(
        logger_,
        ...
        joint.status_word);
      joint.last_status_word = joint.status_word;
    }

    const double new_position =
      counts_to_radians(joint, joint.actual_count);

    if (!joint.initialized ||
        !std::isfinite(joint.previous_position))
    {
      joint.position = new_position;
      joint.previous_position = new_position;
      joint.velocity = 0.0;
      joint.command = new_position;
      joint.applied_command = new_position;
      joint.initialized = true;
      continue;
    }
    ...
    if (dt > 1.0e-6 && dt < 0.1) {
      raw_velocity =
        (new_position - joint.previous_position) / dt;
    }

    constexpr double alpha = 0.15;
    joint.velocity =
      alpha * raw_velocity +
      (1.0 - alpha) * joint.velocity;
  ...
  const rclcpp::Duration & period)
{
  ...
  if (dt <= 1.0e-6 || dt > 0.1) {
    dt = static_cast<double>(cycle_ns_) / 1.0e9;
  }
  ...
  for (auto & joint : joints_) {
    ...
    if (!enable_drives_ || safety_stop_) {
      ...
      if (!std::isfinite(joint.applied_command)) {
        joint.applied_command = joint.position;
      }

      const double desired = clamp_double(
        joint.command,
        joint.lower,
        joint.upper);
      ...
      joint.applied_command = clamp_double(
        desired,
        joint.applied_command - maximum_step,
        joint.applied_command + maximum_step);

      const uint16_t state =
        decode_drive_state(joint.status_word);

      if (state == STATE_OPERATION_ENABLED &&
          abs_double(joint.applied_command - joint.position) >
            max_following_error_rad_)
      {
        RCLCPP_ERROR(
          logger_,
          "%s跟随误差超过限制：command=%.6f actual=%.6f limit=%.6f",
          joint.name.c_str(),
        ...
        joint.target_count =
          radians_to_counts(joint, joint.applied_command);
      }
    }

    joint.control_word = next_control_word(joint);
